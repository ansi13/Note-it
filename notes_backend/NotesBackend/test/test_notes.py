...
def test_list_all_notes(client, notes_fixture):
    ...
    username = \
        token_validation.validate_token_header(headers['Authorization'],
                                               PUBLIC_KEY)
...
def test_view_note(client, notes_fixture):
    ...
    username = \
        token_validation.validate_token_header(headers['Authorization'],
                                               PUBLIC_KEY)
    response = client.get(f'/api/me/notes/{note_ids[0]}/',
                          headers=headers)
    result = response.json
...
def test_view_note_existing_unauthorized(client, notes_random_fixture):
    ...
    header = \
        token_validation.generate_token_header('dummy user',
                                               PRIVATE_KEY)
    headers = {
        'Authorization': header,
    }
...
def test_update_note(client, notes_fixture):
    ...
    response = client.post(f'/api/me/notes/{note_ids[1]}/', data=text,
                           headers=headers)
    old_result = response.json
    assert http.client.OK == response.status_code

    new_response = client.get(f'/api/me/notes/{note_ids[1]}/',
                              headers=headers)
...
def test_update_note_existing_unauthorized(client, notes_random_fixture):
    ...
    response = client.post(f'/api/me/notes/{note_ids[1]}/', data=text,
                          headers=headers)
    old_result = response.json
    assert (http.client.NOT_FOUND == response.status_code or
...
def test_get_non_existent_note(client):
    ...
    response = client.get(f'/api/me/notes/{note_id}/',
                          headers=headers)
